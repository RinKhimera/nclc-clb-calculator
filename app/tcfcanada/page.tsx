...
const TcfCanada = () => {
  ...
  const onSubmit = (values: z.infer<typeof tcfSchema>) => {
    let listeningScore = listeningNCLC(values.listening)
    let readingScore = readingNCLC(values.reading)
    let speakingScore = speakingWritingNCLC(values.speaking)
    let writingScore = speakingWritingNCLC(values.writing)
  ...
  const renderNCLCValue = (module: string) => {
    if (NCLClistening && NCLCreading && NCLCspeaking && NCLCwriting) {
      if (module === "Compréhension orale") {
        return (
          <p>
            NCLC <span className="text-pink-600">{NCLClistening}</span>
          </p>
        )
      } else if (module === "Compréhension écrite") {
        return (
          <p>
            NCLC <span className="text-pink-600">{NCLCreading}</span>
          </p>
        )
      } else if (module === "Expression orale") {
        return (
          <p>
            NCLC <span className="text-pink-600">{NCLCspeaking}</span>
          </p>
        )
      } else if (module === "Expression écrite") {
        return (
          <p>
            NCLC <span className="text-pink-600">{NCLCwriting}</span>
          </p>
        )
      }
    } else {
